...
if st.session_state.predicted is not None:
    ...
    st.info(f"Calorie Gap: {st.session_state.gap - st.session_state.total_consumed:.1f} kcal")

    if st.session_state.gap - st.session_state.total_consumed <= 0:
        st.info("No additional snack needed.")

# ======================
# SNACK OPTIONS MENU
# ======================
if st.session_state.predicted is not None and st.session_state.gap - st.session_state.total_consumed > 0:
    option = st.radio(
        "Choose snack option:",
        (
    ...
    # -------- OPTION 4 --------
        ...
        items_input4 = st.number_input("Items eaten", 0.0, 50.0, 1.0, key="option4")
        energy = items_input4 * row["Serving_Size"] * row["Energy_kcal_g"]
        remaining_after = remaining_gap - energy

        st.write(f"{food_name}: {energy:.1f} kcal")
        st.write(f"Remaining gap: {remaining_after:.1f} kcal")
        food_portion_atlas(row)
        macro_pie_chart(row)

    # -------- OPTION 5 --------
    elif option.startswith("5"):
        items_input5 = st.number_input("Items eaten", 0.0, 50.0, 1.0, key="option5")
        energy = items_input5 * row["Serving_Size"] * row["Energy_kcal_g"]
        st.write(f"{food_name}: {energy:.1f} kcal")
        food_portion_atlas(row)
        macro_pie_chart(row)
