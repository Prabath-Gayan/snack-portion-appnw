...
if st.session_state.predicted is not None and remaining_gap > 0:
    ...
    # -----------------
    # OPTION 4: MULTIPLE SNACKS UNTIL ENERGY FULFILLED
    # -----------------
    elif option.startswith("4"):
        st.write(f"Remaining gap: {remaining_gap:.1f} kcal")
        st.write("Add multiple snacks until energy is fulfilled:")

        add_snack = st.selectbox("Add Snack", df_snacks["Name"].unique(), key="opt4_add")
        row_add = df_snacks[df_snacks["Name"] == add_snack].iloc[0]
        quantity = st.number_input("Items eaten", 0.0, 50.0, 1.0, key="opt4_qty")

        if st.button("Add Snack to Option 4", key="opt4_button"):
            energy = quantity * row_add["Serving_Size"] * row_add["Energy_kcal_g"]
            st.session_state.total_consumed += energy
            st.session_state.option4_list.append((add_snack, quantity, energy))
            st.success(f"Added {quantity} items of {add_snack} ({energy:.1f} kcal)")

        # Display current added snacks
        if st.session_state.option4_list:
            st.write("### Snacks Added (Option 4)")
            for f, q, e in st.session_state.option4_list:
                st.write(f"{f}: {q} items → {e:.1f} kcal")
            new_remaining = st.session_state.gap - st.session_state.total_consumed
            st.write(f"Remaining gap: {max(new_remaining,0):.1f} kcal")

    # -----------------
    # OPTION 5: MULTIPLE SNACKS WITHOUT ENERGY LIMIT
    # -----------------
    elif option.startswith("5"):
        st.write("Add multiple snacks with quantities (no energy limit):")

        add_snack5 = st.selectbox("Add Snack", df_snacks["Name"].unique(), key="opt5_add")
        row_add5 = df_snacks[df_snacks["Name"] == add_snack5].iloc[0]
        quantity5 = st.number_input("Items eaten", 0.0, 50.0, 1.0, key="opt5_qty")

        if st.button("Add Snack to Option 5", key="opt5_button"):
            st.session_state.option5_list.append((add_snack5, quantity5))
            st.success(f"Added {quantity5} items of {add_snack5}")

        # Display current added snacks
        if st.session_state.option5_list:
            st.write("### Snacks Added (Option 5)")
            for f, q in st.session_state.option5_list:
                st.write(f"{f}: {q} items")
